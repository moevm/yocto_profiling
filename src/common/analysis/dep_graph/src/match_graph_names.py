...
def match(node_name, sorted_nodes):
    ...
    variants = list(filter(lambda x: x[0].startswith(pkg_name), sorted_nodes)) # all tasks for the package(s)
    task_variant = list(filter(lambda x: x[1] == task_type, variants)) # we search among them for the required task
    ...
    else: # if not found, it means this task was not in the statistics
        return -1, -1, -1
    return -1, -1, -1
