...
я посмотрела сколько весит файл hashserv.db, который хранит информацию о данном маппинге, на моем хэш-сервере, который я использую, размер файла был 57,3кБ. Затем я удалила всю директорию, в которой запускала хэш-сервер, и пересоздала заново по инструкции из https://github.com/moevm/os_profiling/blob/main/wiki/yocto_cache/setup_OEEquivHash_server.md. После запуска команды bitbake-hashserv был вновь сгенерирован файл hashserv.db, который сразу после создания (до начала какой-либо сборки) весил 57,3кБ.

Изучив подробнее, что хранится в файле hashserv.db после его создания, было выяснено, что в таком состоянии в базе данных находится только инициализация базы данных (таблички без записей). То есть, если вы видите, что ваш файл hashserv.db весит 57,3кБ, значит никакой информации о маппинге в нём нет, по сути база пуста. 

Был проведен еще ряд экспериментов, в ходе которых было выяснено, что:
1. База hashserv.db заполняется в процессе сборки
2. После полноценной новой сборки вес файла был около 2МБ, и там действительно хранился маппинг хэшей, который нам и нужен.
3. Запуск хэш-сервера по вышеуказанной инструкции подразумевает запуск в read-only режиме, то есть в такой конфигурации не стоит ожидать, что в пустой базе после инициализации появится информация о маппинге хэшей.
4. Когда была запущена новая чистая сборка при включенном хэш-сервере в обычном (не read-only) режиме и настроенном зеркале, файл hashserv.db, лежащий на хэш-сервере, так и остался пустым (без маппинга), зато заполнился файл hashserv.db, лежащий в /build/cahce. Затем файл из /build/cache был скопирован на хэш-сервер, и тогда мы получили работающий с маппингом хэш-сервер.
